...
pub fn convert_bpm(
    ...
    shared::process_files(file_path, is_folder, tx, cancel.clone(), |pdf, out_dir, name, prog_tx| {
        ...
        // Reverting exactly to Python logic: 3000Hz framerate, hard square pulses
        ...
        let tmp = tmp_dir.join(format!("{name}_tmp.wav"));
        {
            ...
            // Helper matching python's exact write structure
            let write_n = |w: &mut hound::WavWriter<_>, sample: i16, n: usize, written: &mut usize| -> Result<(), String> {
                for _ in 0..n {
                    ...
                    w.write_sample(sample).map_err(|e| e.to_string())?; // channel 2
                    *written += 1;
                }
                Ok(())
            };
        ...
        let args: Vec<String> = vec![
            ...
            "-vn".into(), "-ar".into(), "48000".into(), "-ac".into(), "2".into(), // Output at 48k just like python
